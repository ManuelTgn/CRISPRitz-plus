...
ProfileAccumulator::ProfileAccumulator(std::string guide, int guide_len,
                                       int pam_len, int max_mm,
                                       int max_bulge_dna, int max_bulge_rna,
                                       bool pam_at_start)
    : guide_(std::move(guide)), guide_len_(guide_len), pam_len_(pam_len),
      max_mm_(max_mm), max_bulge_dna_(max_bulge_dna),
      max_bulge_rna_(max_bulge_rna), pam_at_start_(pam_at_start) {
  if (guide_len_ <= 0)
  ...
  // File 2
  ext_mm_nuc_pos_.assign(mm_slots, std::vector<std::vector<int>>(
                                       4, std::vector<int>(guide_len_, 0)));
...
int ProfileAccumulator::nuc_index(char c) noexcept {
  ...
  case 'T':
    return 3;
  default:
    return -1; // 'N' or any ambiguous base
  }
}
...
void ProfileAccumulator::push(const OffTarget &ot) {
  ...
  const std::size_t body_start =
      pam_at_start_ ? static_cast<std::size_t>(pam_len_) : 0u;
  const std::size_t body_end =
      pam_at_start_ ? aln_len : aln_len - static_cast<std::size_t>(pam_len_);
  ...
  for (std::size_t i = body_start; i < body_end; ++i) {
    ...
    if (g == '-') {
      ...
      const char gu =
          static_cast<char>(std::toupper(static_cast<unsigned char>(g)));
      const char tu =
          static_cast<char>(std::toupper(static_cast<unsigned char>(t)));
      ...
      if (is_mismatch && body_pos < guide_len_) {
        ...
        if (has_dna)
          pos_mm_in_dna_[body_pos]++;
        if (has_rna)
          pos_mm_in_rna_[body_pos]++;
...
void ProfileAccumulator::reset() {
  ...
  (void)dna_slots;
  (void)rna_slots; // used only at construction
}
